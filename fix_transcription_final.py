...
Ce script corrige le fichier merger.py pour assurer que les transcriptions sont trouvees
"""
...
def main():
    ...
    try:
        ...
        # 1. Ajouter la methode de chargement des fichiers texte
        load_text_files_code = '''
    def _load_transcription_text_files(self):
        ...
        # 4. Code pour améliorer le traitement des fichiers sans extension
        replace_improve_code = '''audio_file = match.group(1).strip()
        ...
        # 4. Améliorer la méthode _replace_audio_references_exact
        replace_pattern = r"audio_file = match\.group\(1\)\.strip\(\)"
        if re.search(replace_pattern, content):
            content = re.sub(replace_pattern, replace_improve_code, content)
        else:
