...
"""
Script corrigé de reconstruction complète des exports
...
Version corrigée pour traiter le format exact des dates et références audio.
"""
...
def rebuild_received_messages_file():
    ...
    for contact in contact_folders:
        ...
        try:
            ...
            for line in lines:
                if '='*20 in line and not in_messages:
                    in_messages = True
                    continue
...
def find_audio_references_in_file(file_path):
    ...
    try:
        ...
        # Pattern pour trouver les références audio et les UUID - FORMAT CORRIGÉ
        # Format: [YYYY/MM/DD HH:MM] ← [AUDIO] received_UUID_1.opus
        pattern = r'\[([\d/]+)\s+([\d:]+)\]\s*←\s*\[AUDIO\]\s+([^\n\[\]]+)'
    ...
    try:
        ...
        for i in range(1, len(sections), 2):
            if i + 1 < len(sections):
                ...
                for line in lines:
                    # Nouvelle ligne de message - FORMAT CORRIGÉ: [YYYY/MM/DD HH:MM]
...
def analyze_format(file_path):
    """Analyse le format exact des messages dans un fichier pour aider au débogage"""
    logger.info(f"Analyse du format de {os.path.basename(file_path)}")
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.readlines()
            
        # Analyser les 20 premières lignes commençant par [
        examples = []
        count = 0
        for line in content:
            if line.strip().startswith('[') and not line.strip().startswith('[TRANSCRIPTION]'):
                examples.append(line.strip())
                count += 1
                if count >= 20:
                    break
        
        logger.info("Exemples de format de messages:")
        for i, example in enumerate(examples):
            logger.info(f"  {i+1}. {example}")
            
        # Analyser spécifiquement les messages audio
        audio_messages = [line for line in content if '[AUDIO]' in line][:10]
        logger.info(f"Trouvé {len(audio_messages)} messages audio (exemples):")
        for i, msg in enumerate(audio_messages[:5]):
            logger.info(f"  {i+1}. {msg.strip()}")
            
        # Vérifier la présence de transcriptions
        transcriptions = [line for line in content if '[TRANSCRIPTION]' in line][:10]
        logger.info(f"Trouvé {len(transcriptions)} transcriptions (exemples):")
        for i, trans in enumerate(transcriptions[:5]):
            logger.info(f"  {i+1}. {trans.strip()}")
            
        return True
    except Exception as e:
        logger.error(f"Erreur lors de l'analyse du format: {str(e)}")
        return False

def main():
    """Point d'entrée principal"""
    logger.info("="*50)
    logger.info("RECONSTRUCTION COMPLÈTE DES EXPORTS - VERSION CORRIGÉE")
    logger.info("="*50)
    
    # 0. Analyser un fichier de contact pour comprendre le format exact
    logger.info("\n=== ÉTAPE 0: ANALYSE DU FORMAT DES FICHIERS ===")
    sample_contact = "_41_76_628_52_57"  # Contact que nous avons déjà examiné
    sample_file = os.path.join(OUTPUT_DIR, sample_contact, "messages_recus.txt")
    if os.path.exists(sample_file):
        analyze_format(sample_file)
    else:
        logger.warning(f"Fichier exemple introuvable: {sample_file}")
    ...
    # Analyse du format du fichier global généré
    logger.info("Analyse du format du fichier global généré:")
    analyze_format(messages_file)
