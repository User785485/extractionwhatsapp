...
def force_fix_files():
    ...
    print("=== CORRECTION FORCEE DES FICHIERS ===")
    ...
    try:
        with open(test_file, 'w') as f:
            f.write("Test d'ecriture")
        if os.path.exists(test_file):
            os.remove(test_file)
            print("OK - Permissions d'ecriture OK")
    ...
    for root, _, files in os.walk(output_dir):
        for filename in files:
            if filename.endswith('.txt'):
                ...
                if size > 1000000:
                    print(f"Fichier trouve: {filename} ({size:,} octets)")
                    file_paths[filename] = {
                        'path': full_path,
                        'size': size
                    }
    ...
    if file_paths:
        ...
        if conversations_files:
            ...
            for target in ['toutes_conversations.txt', 'toutes_conversations_avec_transcriptions.txt']:
                ...
                try:
                    ...
                    print(f"OK - Fichier cree: {target} ({os.path.getsize(target_path):,} octets)")
                except Exception as e:
        ...
        print("! Aucun fichier source de taille correcte trouve")
        return False
    ...
    for filename in target_files:
        ...
        if os.path.exists(filepath):
            ...
            if size > 1000000:
                ...
                print(f"NOK - {filename}: Trop petit ({size:,} octets)")
                all_ok = False
        else:
            print(f"NOK - {filename}: MANQUANT")
            all_ok = False
    ...
    if all_ok:
        ...
        print("\nCertains fichiers n'ont pas pu etre corriges.")
        return False
