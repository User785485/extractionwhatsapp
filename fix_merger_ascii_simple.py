...
# -*- coding: utf-8 -*-
"""
Diagnostic complet pour comprendre pourquoi l'export echoue
VERSION ASCII PURE
"""
...
def check_all_files():
    """Verifie tous les fichiers importants"""
    output_dir = r"C:\Datalead3webidu13juillet"
    print(f"\n=== DIAGNOSTIC COMPLET DU REPERTOIRE {output_dir} ===")
    print(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    # 1. Verifier les fichiers de base
    ...
    for filename in base_files:
        ...
        if os.path.exists(filepath):
            ...
            print(f"[OK] {filename}: {size} octets")
            if size > 1000:
                found_base = True
                # Afficher un echantillon
                with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                    sample = f.read(500)
                    print(f"  Echantillon: {sample[:100]}...")
        else:
            print(f"[MANQUE] {filename}: MANQUANT")
    
    if not found_base:
        print("\n[ERREUR] PROBLEME: Aucun fichier de base valide trouve!")
    
    # 2. Verifier les fichiers fusionnes
    print("\n2. FICHIERS FUSIONNES:")
    print("-" * 50)
    fused_files = [
    ...
    for filename in fused_files:
        ...
        if os.path.exists(filepath):
            size = os.path.getsize(filepath)
            print(f"{'[OK]' if size > 1000 else '[ATTENTION]'} {filename}: {size} octets")
            if size < 1000:
        ...
        for file in os.listdir(mapping_dir):
            if file.endswith('_mappings.json'):
                ...
                try:
                    ...
                    pass
        print(f"[OK] Total transcriptions dans les mappings: {total_transcriptions}")
    else:
        print("[MANQUE] Repertoire de mappings manquant")
    
    # 4. Verifier un contact au hasard
    ...
    try:
        ...
        if contacts:
            ...
            # Verifier les fichiers du contact
            for filename in ['tous_messages.txt', 'messages_recus.txt']:
                ...
                if os.path.exists(filepath):
                    ...
                    with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        if audio_refs:
                            print(f"    - {len(audio_refs)} references [AUDIO] trouvees")
...
def create_manual_fix():
    """Cree un script de correction manuelle"""
...
'''
    ...
    print("\n[OK] Script de correction cree: manual_fix.py")

def analyze_merger_issue():
    """Analyse pourquoi le merger echoue"""
    print("\n5. ANALYSE DU PROBLEME DU MERGER:")
    print("-" * 50)
    
    # Verifier si le merger trouve les bons fichiers
    output_dir = r"C:\Datalead3webidu13juillet"
    ...
    if not os.path.exists(old_name) and os.path.exists(new_name):
        print("[PROBLEME] PROBLEME IDENTIFIE:")
        ...
        if response == 'y':
            try:
                ...
                print(f"[OK] Fichier renomme avec succes!")
            except Exception as e:
...
def main():
    ...
    # 3. Creer le script de correction
    create_manual_fix()
    
    print("\n=== SOLUTIONS PROPOSEES ===")
    print("\n1. CORRECTION MANUELLE (recommande):")
    ...
    print("\n3. SI TOUT ECHOUE, utiliser l'export robuste:")
